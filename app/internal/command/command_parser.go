...
func parseXaddCommand(command *Command) (XaddCommand, error) {
	...
	for n, arg := range command.CommandValues {
		if n == 0 {
			...
			if sequenceNumber == -1 {
				xaddCommand.AutogeneratedSequenceNumber = true
			} else {
				xaddCommand.EntryIdSequenceNumber = sequenceNumber
			}

			if millisecondsTime == -1 {
				xaddCommand.AutogeneratedMillisecondsTime = true
			} else {
...
func parseXaddCommandEntryId(entryId string) (int, int, error) {
	...
	if entryIdMillisecondsTime == "*" {
		...
		millisecondsTime, err = strconv.Atoi(entryIdMillisecondsTime)
		if err != nil {
