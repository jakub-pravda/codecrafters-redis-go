...
func (c XaddCommand) Process() (respparser.RespContent, error) {
	...
	if c.AutogeneratedSequenceNumber {
		// Handle autogenerated sequence number
		if topItemFound && topItem.EntryIdMillisecondsTime == c.EntryIdMillisecondsTime {
			sequenceNumber = topItem.EntryIdSequenceNumber + 1
		} else if c.EntryIdMillisecondsTime == 0 {
			sequenceNumber = 1
		} else {
			sequenceNumber = 0
		}
	...
	resp := respparser.RespContent{
		Value:    fmt.Sprintf("%d-%d", c.EntryIdMillisecondsTime, sequenceNumber),
		DataType: respparser.BulkString,
	}
